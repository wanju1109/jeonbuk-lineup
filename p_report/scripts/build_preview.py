...
def build_preview_payload(
    ...
    if JEONBUK not in f"{home}{away}":
        return _build_neutral_preview(row, catalog, index_matches, kickoff, published)
    return _build_jeonbuk_preview(row, catalog, index_matches, kickoff, published)
...
def _build_jeonbuk_preview(
    ...
    jb_form = recent_form_catalog(catalog, JEONBUK, 5)
    opp_form = recent_form_catalog(catalog, opponent, 5)
...
def _build_neutral_preview(
    ...
    home_form = recent_form_catalog(catalog, home, 5)
    away_form = recent_form_catalog(catalog, away, 5)
...
def main() -> None:
    ...
    for row, kickoff, published in sorted(merged.values(), key=lambda x: x[1]):
        preview = build_preview_payload(row, catalog, index_matches, kickoff, published)
