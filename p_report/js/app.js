(() => {
  ...
  function renderForm(list, box, detailed = false) {
    ...
    box.innerHTML = list
      .map((r) => {
        ...
        let stats = "";
        if (detailed && r.xg != null && !r.stats_limited) {
          stats = `<span class="form-stats">xG ${escapeHtml(String(r.xg ?? "-"))} · ${escapeHtml(String(r.shots ?? "-"))}슈팅</span>`;
        } else if (detailed && r.stats_limited) {
          stats = `<span class="form-stats muted">K리그 포털에서 집계 중</span>`;
        }
        return (
          `<div class="form-row">` +
  ...
  function formForSide(preview, side) {
    const meta = preview.meta || {};
    const form = preview.form || {};
    const isJb = meta.jeonbuk_match !== false && isJeonbukMatch(meta);
    if (!isJb) {
      return side === "home" ? form.jeonbuk || [] : form.opponent || [];
    }
    const homeName = meta.home?.name || "";
    const awayName = meta.away?.name || "";
    const target = side === "home" ? homeName : awayName;
    return target.includes(JEONBUK) ? form.jeonbuk || [] : form.opponent || [];
  }

  function styleForSide(preview, side) {
    const meta = preview.meta || {};
    const style = preview.style || {};
    const isJb = meta.jeonbuk_match !== false && isJeonbukMatch(meta);
    if (!isJb) {
      return side === "home" ? style.jeonbuk || {} : style.opponent || {};
    }
    const homeName = meta.home?.name || "";
    const awayName = meta.away?.name || "";
    const target = side === "home" ? homeName : awayName;
    return target.includes(JEONBUK) ? style.jeonbuk || {} : style.opponent || {};
  }
  ...
  function renderMatchup(matchup, meta) {
    ...
    const isJb = meta?.jeonbuk_match !== false && isJeonbukMatch(meta);
    const jbIsHome = homeName.includes(JEONBUK);
    if (!rows.length) {
    ...
    box.innerHTML =
      `<div class="stat-head">` +
      `<span></span><span class="stat-team-a">${escapeHtml(homeName)}</span><span class="stat-team-b">${escapeHtml(awayName)}</span>` +
      `</div>` +
      rows
        .map((row) => {
          ...
          let winAway = false;
          if (isJb) {
            valHome = jbIsHome ? row.jeonbuk : row.opponent;
            valAway = jbIsHome ? row.opponent : row.jeonbuk;
            if (row.better === "jeonbuk") {
              winHome = jbIsHome;
              winAway = !jbIsHome;
            } else if (row.better === "opponent") {
              winHome = !jbIsHome;
              winAway = jbIsHome;
            }
          } else {
            valHome = row.jeonbuk;
            valAway = row.opponent;
            winHome = row.better === "jeonbuk";
            winAway = row.better === "opponent";
          }
          const clsHome = winHome ? " stat-win stat-win-a" : "";
          const clsAway = winAway ? " stat-win stat-win-b" : "";
          return (
            `<div class="stat-row">` +
            `<span class="stat-label">${escapeHtml(row.label || "")}</span>` +
            `<span class="stat-val${clsHome}">${escapeHtml(String(valHome ?? "-"))}${escapeHtml(suf)}</span>` +
            `<span class="stat-val${clsAway}">${escapeHtml(String(valAway ?? "-"))}${escapeHtml(suf)}</span>` +
            `</div>`
          );
        })
        .join("");
  ...
  function renderPreview(preview) {
    ...
    $("styleBox").innerHTML = styleCard(homeStyle) + styleCard(awayStyle);

    if ($("jbFormTitle")) {
      $("jbFormTitle").textContent = `${meta.home?.name || "홈"} 최근`;
    }
    $("oppFormTitle").textContent = `${meta.away?.name || "원정"} 최근`;
    renderForm(formForSide(preview, "home"), $("jbForm"), true);
    renderForm(formForSide(preview, "away"), $("oppForm"), true);
