...
# Default Jeonbuk. Use "*", "ALL", or "any" (or explicit empty) for any club.
_raw_team = os.environ.get("KLEAGUE_TEAM", "전북")
TEAM_FILTER = "" if str(_raw_team).strip() in ("", "*", "ALL", "any") else str(_raw_team).strip()
...
def save_index(matches: list[dict]) -> None:
    ...
    for m in matches:
        ...
        if not gid:
            continue
        # Main index stays Jeonbuk-only so the top picker stays 1 match per round.
...
def save_schedule(rows: list[dict], year: str, meet_seq: str) -> None:
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    by_key = {}
...
def build_schedule_rows(
    client: PortalClient, year: str, meet_seq: str, round_ids: list[str]
) -> list[dict]:
    rows: list[dict] = []
    for rid in round_ids:
        try:
            ...
            print(f"[WARN] schedule round {rid} failed: {exc}")
            continue
            ...
            rows.append(
                {
                    ...
                    "home": home,
                    "away": away,
                    "end_yn": match.get("end_yn") or "N",
                    "label": match.get("label") or f"{home} VS {away}",
                    "date_md": match.get("date_md") or "",
                }
            )
        time.sleep(0.15)
    return rows
...
def _cell_text(blob: str) -> str:
    return _clean_html_text(re.sub(r"<[^>]+>", "", blob or ""))


def parse_pass_matrix_table(html: str, table_id: str) -> dict | None:
    """Parse the PASS MATRIX HTML table (homeList / awayList)."""
    m = re.search(
    ...
    if not m:
        return None
    table = m.group(1)
    thead = re.search(r"<thead>(.*?)</thead>", table, flags=re.I | re.S)
    receivers: list[str] = []
    if thead:
        ths = re.findall(r"<th[^>]*>(.*?)</th>", thead.group(1), flags=re.I | re.S)
        receivers = [_cell_text(t) for t in ths[3:]]
    tbody = re.search(r"<tbody>(.*?)</tbody>", table, flags=re.I | re.S)
    if not tbody:
        return {"players": [], "receivers": receivers, "edges": []}
    players: list[dict] = []
    edges: list[dict] = []
    for tr in re.finditer(r"<tr[^>]*>(.*?)</tr>", tbody.group(1), flags=re.I | re.S):
        tds = re.findall(r"<td[^>]*>(.*?)</td>", tr.group(1), flags=re.I | re.S)
        if len(tds) < 3:
            continue
        back_no = _cell_text(tds[0])
        name = _cell_text(tds[1])
        position = _cell_text(tds[2])
        players.append({"back_no": back_no, "name": name, "position": position})
        for col_i, cell in enumerate(tds[3:]):
            txt = _cell_text(cell)
            if not txt:
                continue
            try:
            ...
            if count <= 0:
                continue
            to_name = receivers[col_i] if col_i < len(receivers) else ""
            if not to_name or to_name == name:
                continue
            edges.append({"from": name, "to": to_name, "from_back": back_no, "count": count})
    edges.sort(key=lambda e: e["count"], reverse=True)
    return {"players": players, "receivers": receivers, "edges": edges}


def parse_pass_matrix(html: str) -> dict | None:
    home = parse_pass_matrix_table(html, "homeList")
    away = parse_pass_matrix_table(html, "awayList")
    if not home and not away:
        return None
    return {
        "home": home or {"players": [], "receivers": [], "edges": []},
        "away": away or {"players": [], "receivers": [], "edges": []},
    }
...
def fetch_chalkboard(client: PortalClient, year: str, meet_seq: str, round_id: str, game_id: str) -> dict:
    ...
    try:
        ...
        pass_matrix = parse_pass_matrix(pm_html)
    except Exception as exc:
        print(f"[WARN] pass matrix fetch failed game_id={game_id}: {exc}")
...
def main() -> None:
    ...
    # If only one round/game was requested, still refresh the full-year schedule once.
    if ROUND or GAME_ID:
        try:
            all_rounds = fetch_rounds(client, YEAR, MEET_SEQ)
            schedule_rows = build_schedule_rows(client, YEAR, MEET_SEQ, all_rounds)
        except Exception as exc:
            print(f"[WARN] full schedule refresh failed: {exc}")

    save_schedule(schedule_rows, YEAR, MEET_SEQ)
    save_index(list(by_id.values()))
    save_collected()
    print(f"[DONE] collected={collected}, index={len(by_id)}, schedule={len(schedule_rows)}")
