...
const DeepView = (() => {
  const HOME_ACCENT = "#d6f56a";
  const AWAY_ACCENT = "#ffb08a";
  const HOME_LINE = "#146b4a";
  const AWAY_LINE = "#e4572e";
  ...
  function drawShape(canvasId, team, accent) {
  ...
  function renderShapes(ctx) {
    ...
    drawShape("ddHomeShape", ctx.home, HOME_ACCENT);
    drawShape("ddAwayShape", ctx.away, AWAY_ACCENT);
  ...
  function drawMomentum(ctx) {
    ...
    const series = [
      { key: "home", color: HOME_LINE, name: ctx.homeName },
      { key: "away", color: AWAY_LINE, name: ctx.awayName },
    ];
    ...
    for (const goal of ctx.momentum.goals) {
      ...
      const color = goal.side === "home" ? HOME_LINE : AWAY_LINE;
    ...
    if ($("ddMomentumLegend")) {
      $("ddMomentumLegend").innerHTML =
        `<span class="dd-legend-item"><i style="background:${HOME_LINE}"></i>${escapeHtml(ctx.homeName)}</span>` +
        `<span class="dd-legend-item"><i style="background:${AWAY_LINE}"></i>${escapeHtml(ctx.awayName)}</span>`;
    }
  }
  ...
  function zoneTable(team, name, accentRgb) {
    ...
    for (let li = 0; li < lanes.length; li += 1) {
      for (let ti = 0; ti < rows.length; ti += 1) {
        ...
        cells.push(
          `<div class="dd-zone-cell" style="background:rgba(${accentRgb},${(0.08 + intensity * 0.82).toFixed(3)})" title="${escapeHtml(
  ...
  function renderZones(ctx) {
    ...
    if (!box) return;
    box.innerHTML =
      zoneTable(ctx.home, ctx.homeName, "20,107,74") + zoneTable(ctx.away, ctx.awayName, "228,87,46");
  }
  ...
  function renderChapters(ctx, chapters) {
    ...
    box.innerHTML = chapters
      .map((ch) => {
        ...
        return (
          `<article class="dd-chapter" id="dd-${escapeHtml(ch.id)}">` +
  ...
  function bindRoleTabs(ctx) {
    ...
    if (homeBtn) homeBtn.textContent = ctx.homeName;
    if (awayBtn) awayBtn.textContent = ctx.awayName;
  }
  ...
  function redrawCanvases() {
    ...
    drawShape("ddHomeShape", currentCtx.home, HOME_ACCENT);
    drawShape("ddAwayShape", currentCtx.away, AWAY_ACCENT);
