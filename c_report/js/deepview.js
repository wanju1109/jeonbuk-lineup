...
const DeepView = (() => {
  ...
  function shapeSubtitle(team, name) {
    ...
    const parts = [];
    if (f.label) parts.push(`보이는 형태 ${f.label}`);
    parts.push(`평균 위치 ${fmt(team.shape.blockHeight)}`);
    parts.push(`앞뒤 ${fmt(team.shape.depthSpread)} · 좌우 ${fmt(team.shape.widthSpread)}`);
  ...
  function renderShapes(ctx) {
    ...
    if ($("ddShapeNote")) {
      $("ddShapeNote").textContent =
        `원의 크기는 공을 만진 횟수입니다. 선의 굵기는 두 선수 사이 패스 횟수입니다. ` +
  ...
  function drawMomentum(ctx) {
    ...
    if ($("ddMomentumNote")) {
      $("ddMomentumNote").textContent =
        `xG 누적(시간이 흐르며 쌓인 골이 될 확률)입니다. 계단이 클수록 그 순간의 기회가 좋았습니다. ` +
        `점은 실제 득점 시점입니다. 최종 합 ${ctx.homeName} ${fmt(ctx.momentum.homeTotal, 2)} · ${ctx.awayName} ${fmt(ctx.momentum.awayTotal, 2)}.`;
    }
    if ($("ddMomentumLegend")) {
  ...
  function matchupTableHtml(table, homeName, awayName) {
    ...
    return (
      ...
      `<caption>상대 골문 앞 30m에 들어간 길과, 그 길을 막아선 수비 횟수입니다. 저항도가 낮을수록 상대가 그 길을 내줬다는 뜻입니다.</caption>` +
      `<thead><tr>` +
      `<th scope="col">길</th>` +
  ...
  const ROLE_GLOSSARY = [
    {
      ...
      items: [
        {
          term: "골키퍼",
          desc: "골문을 지키며 마지막 수비를 담당합니다. 세이브·펀칭·골킥 등 골키퍼 행동이 기록됩니다.",
        },
      ],
    },
    {
      ...
      items: [
        {
          term: "센터백",
          desc: "수비 라인 가운데. 상대 공격을 막고 공을 걷어내거나, 짧은 패스로 공격을 시작합니다.",
        },
        {
          term: "볼플레잉 센터백(전진형 센터백)",
          desc: "일반 센터백보다 앞에서 압박하거나 패스를 시작합니다. 팀이 높은 수비 라인을 유지할 때 자주 보입니다.",
        },
        {
          term: "풀백",
          desc: "측면 수비수. 상대 윙을 막고, 필요할 때 측면으로 올라가 크로스를 돕습니다. (왼쪽/오른쪽)",
        },
        {
          term: "오버래핑 풀백(공격형 풀백)",
          desc: "풀백이지만 상대 골문 근처까지 자주 올라가 크로스나 컷백을 만듭니다. (왼쪽/오른쪽)",
        },
      ],
    },
    {
      ...
      items: [
        {
          term: "홀딩 미드필더(수비형 미드필더)",
          desc: "수비 앞에서 공을 받아 연결하거나, 상대 공격을 끊는 거점입니다.",
        },
        {
          term: "딥라잉 플레이메이커(뒤에서 키패스하는 미드)",
          desc: "수비 라인 앞 깊은 자리에서 공을 받아 앞으로 결정적인 패스를 넣는 역할입니다.",
        },
        {
          term: "중앙 미드필더",
          desc: "가운데에서 공을 연결하며 템포를 조절합니다. 공격과 수비 사이를 오갑니다.",
        },
        {
          term: "박스 투 박스(수비부터 공격까지 뛰는 미드)",
          desc: "자기 박스부터 상대 박스까지 넓게 뛰며 압박, 역습, 2선 슈팅을 모두 합니다.",
        },
        {
          term: "미드필더",
          desc: "측면 중원에서 패스 연결과 전환을 담당합니다. (왼쪽/오른쪽)",
        },
        {
          term: "공격형 미드필더",
          desc: "중앙에서 더 앞쪽에 서서 마지막 패스, 침투, 슈팅을 만듭니다.",
        },
        {
          term: "윙어",
          desc: "측면에서 공을 잡고 돌파·크로스·안쪽 컷인을 시도합니다. (왼쪽/오른쪽)",
        },
        {
          term: "인사이드 윙어(안으로 접는 윙어)",
          desc: "측면에서 시작해 안쪽으로 들어와 슈팅이나 키패스를 만드는 역할입니다. (왼쪽/오른쪽)",
        },
        {
          term: "하프스페이스 공격형 미드(안쪽 공격형 미드)",
          desc: "골문과 측면 사이 공간에서 공을 받아 침투하거나 연결하는 역할입니다. (왼쪽/오른쪽)",
        },
      ],
    },
    {
      ...
      items: [
        {
          term: "최전방 스트라이커",
          desc: "가장 앞에서 상대 수비를 붙잡고, 박스 안 마무리와 첫 압박을 담당합니다.",
        },
        {
          term: "처진 스트라이커",
          desc: "공격수지만 한 발 뒤에서 연결, 전환, 2선 슈팅을 만듭니다.",
        },
        {
          term: "윙 포워드",
          desc: "측면 최전방에서 돌파·크로스·1대1로 수비를 끌어냅니다. (왼쪽/오른쪽)",
        },
      ],
    },
  ];
  ...
  function renderRoleGlossary() {
    const box = $("ddRoleGlossary");
    if (!box || box.dataset.ready === "1") return;
    box.dataset.ready = "1";
    box.innerHTML =
      `<p class="dd-role-glossary-lead">카드에 붙는 역할은 평균 위치와 그날 행동으로 자동 추정한 이름입니다. 등록 포지션과 다를 수 있습니다.</p>` +
      `<div class="dd-role-glossary-body">` +
      ROLE_GLOSSARY.map(
        (group) =>
          `<section class="dd-role-glossary-group">` +
          `<h5 class="dd-role-glossary-pos">${escapeHtml(group.pos)}</h5>` +
          `<dl class="dd-role-glossary-list">` +
          group.items
            .map(
              (item) =>
                `<div class="dd-role-glossary-item">` +
  ...
  function render(meta, events, players, lineup, passMatrix) {
    const section = $("deepdive");
    if (!section) return null;
    let ctx;
    let chapters;
    try {
      ctx = Tactics.analyze(meta, events, players, lineup, passMatrix);
