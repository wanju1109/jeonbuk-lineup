...
(() => {
  ...
  function renderAll() {
    ...
    if ($("matchFacts")) {
      $("matchFacts").innerHTML = buildMatchFacts(meta, state.index).join("");
    }
  ...
  function buildMatchFacts(meta, index) {
    ...
    const push = (label, value) => {
      ...
      facts.push(
        `<div class="fact"><span class="fact-label">${escapeHtml(label)}</span><span class="fact-value">${escapeHtml(
          String(value)
        )}</span></div>`
      );
    };
    ...
    push("경기장", meta.venue);
    push("킥오프", meta.kickoff);
    if (meta.attendance != null && meta.attendance !== "") {
      push("관중", `${Number(meta.attendance).toLocaleString("ko-KR")}명`);
    }
    ...
    if (meta.weather) {
      push("날씨", meta.weather);
    } else if (meta.temperature_c != null) {
      push("기온", `${meta.temperature_c}℃`);
    }
    if (meta.humidity != null && meta.humidity !== "") {
      push("습도", `${meta.humidity}%`);
    }
    push("주심", meta.referee);
    const off = meta.officials || {};
    if (off.ar1) push("부심1", off.ar1);
    if (off.ar2) push("부심2", off.ar2);
    if (off.fourth) push("대기심", off.fourth);
    if (off.var) push("VAR", off.var);
    if (meta.home?.manager) push(`${meta.home.name || "홈"} 감독`, meta.home.manager);
    if (meta.away?.manager) push(`${meta.away.name || "원정"} 감독`, meta.away.manager);
    return facts;
  }
  ...
  function renderLineup(meta, lineup) {
    ...
    const renderTeam = (rows) => {
      ...
      const line = (p, tag) => {
        ...
        return `<div class="lineup-row">
          ...
          <span class="ln-pos">${escapeHtml(p.position || tag || "")}</span>
          <span class="ln-name">${escapeHtml(p.name || "")}</span>
          <span class="ln-meta">${escapeHtml(bits.join(" · "))}</span>
        </div>`;
      };
      return (
        starters.map((p) => line(p, "선발")).join("") +
        (bench.length
          ? `<div class="lineup-bench-label">벤치 · 투입</div>` + bench.map((p) => line(p, "벤치")).join("")
          : "")
      );
    };
    ...
    if ($("subList")) {
      if (!sides.subs.length) {
        ...
        $("subList").innerHTML = sides.subs
          .map((s) => {
            const team = s.team_name || (s.ha === "H" ? meta.home.name : meta.away.name);
            const outP = s.player_out
              ? `#${s.player_out.back_no ?? "-"} ${s.player_out.name}`
              : "OUT ?";
            const inP = s.player_in
              ? `#${s.player_in.back_no ?? "-"} ${s.player_in.name}`
              : "IN ?";
            return `<div class="sub-row">
              <span class="sub-time">${escapeHtml(s.time_label || "")}</span>
              <span class="sub-team">${escapeHtml(team)}</span>
              <span class="sub-swap">${escapeHtml(outP)} → ${escapeHtml(inP)}</span>
            </div>`;
          })
          .join("");
      }
