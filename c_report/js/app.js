...
(() => {
  ...
  function renderAll() {
    ...
    story.push({
      label: "초보",
      text: "아래 ‘전력분석 브리핑’에서 영토·빌드업·슈팅 채널·선수 역할을 이어서 읽으면, 숫자만 볼 때보다 경기의 이유가 선명해집니다.",
    });
      ...
      box.innerHTML = briefing.chapters
        .map((ch) => {
          const paras = (ch.paragraphs || [])
            ...
            .join("");
          return (
            `<article class="brief-card" data-kicker="${escapeHtml(ch.kicker || "")}">` +
        ...
        $("subList").innerHTML = `<div class="sub-timeline">${slots
          .map((slot) => {
            return `<div class="sub-slot">
              ...
              ${sideBlock(meta.home.name, slot.home)}
              ${sideBlock(meta.away.name, slot.away)}
            </div>`;
          })
          .join("")}</div>`;
      }
  ...
  function renderPlayers(events, players, meta) {
    ...
    ranked.forEach((p) => {
      const pos = String(p.pos || "기타").toUpperCase();
      if (currentPos !== null && pos !== currentPos) flush();
      currentPos = pos;
      bucket.push(`<button class="player-btn ${
        String(p.player_id) === String(state.selectedPlayerId) ? "active" : ""
      }" data-id="${p.player_id}" type="button">
          <div class="top">
