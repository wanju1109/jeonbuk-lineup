...
(() => {
  ...
  function buildMatchFacts(meta, index) {
    ...
    const makeFact = (label, value) => {
      ...
      return `<div class="fact"><span class="fact-label">${escapeHtml(label)}</span><span class="fact-value">${escapeHtml(
        String(value)
      )}</span></div>`;
    };
    ...
    const matchItems = [
      makeFact("경기장", meta.venue),
      makeFact("킥오프", meta.kickoff),
      makeFact("날씨", meta.weather || (meta.temperature_c != null ? `${meta.temperature_c}℃` : "")),
      makeFact("습도", meta.humidity != null && meta.humidity !== "" ? `${meta.humidity}%` : ""),
    ];
    pushGroup("경기 정보", matchItems);

    const crowdItems = [];
    if (meta.attendance != null && meta.attendance !== "") {
      crowdItems.push(makeFact("관중", `${Number(meta.attendance).toLocaleString("ko-KR")}명`));
    }
    ...
    pushGroup("심판", [
      makeFact("주심", meta.referee),
      makeFact("부심1", off.ar1),
      makeFact("부심2", off.ar2),
      makeFact("대기심", off.fourth),
      makeFact("VAR", off.var),
      makeFact("AVAR", off.avar),
    ]);

    pushGroup("감독", [
      makeFact(meta.home?.name ? `${meta.home.name} 감독` : "홈 감독", meta.home?.manager),
      makeFact(meta.away?.name ? `${meta.away.name} 감독` : "원정 감독", meta.away?.manager),
    ]);
