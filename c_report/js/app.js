...
(() => {
  ...
  function renderAll() {
    ...
    $("kpiGrid").innerHTML = [
      kpi("xG", h.xg, a.xg, "같은 자리에서 100번 때리면 약 몇 골인가"),
    ...
    if (briefing?.metrics?.state?.thesis) {
      ...
      story.push({
        label: "한줄",
        text: `전북의 골이 될 확률 합(${h.xg})이 상대(${a.xg})보다 높았습니다. 기회는 더 좋았지만 스코어는 ${meta.score.home}-${meta.score.away}입니다. 상대는 넣었고, 우리는 마무리가 아쉬웠습니다.`,
      });
    } else {
      story.push({
        label: "한줄",
        text: `최종 스코어 ${meta.score.home}-${meta.score.away}. 슈팅 ${h.shots}-${a.shots}, 골이 될 확률 합 ${h.xg}-${a.xg}. 전반 확률 합 ${periods[1][meta.home.team_id].xg}-${periods[1][meta.away.team_id].xg}, 후반 ${periods[2][meta.home.team_id].xg}-${periods[2][meta.away.team_id].xg}.`,
      });
    }
    if (flow.hasFirstGoal) {
      ...
      story.push({
        label: "흐름",
        text: `선제골은 ${flow.clock}, ${scorer}입니다. 이후 골이 될 확률 합은 ${flow.after[meta.home.team_id].xg} 대 ${flow.after[meta.away.team_id].xg}, 슈팅은 ${flow.after[meta.home.team_id].shots} 대 ${flow.after[meta.away.team_id].shots}입니다.`,
      });
    }
    if (att.available) {
  ...
  function renderDeepDive(meta, events, players, lineup) {
    ...
    try {
      DeepView.render(meta, events, players, lineup);
    } catch (err) {
      console.error(err);
