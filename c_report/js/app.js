...
(() => {
  ...
  function rebuildOtherMatches() {
    ...
    list.forEach((m) => {
      ...
      const done = m.end_yn === "Y" ? "" : " (미종료)";
      opt.textContent = vs + done;
  ...
  async function fetchOtherAndLoad() {
    ...
    try {
      ...
      if (!selected) {
        showOtherReportMessage("경기를 먼저 선택해 주세요.", true);
        return;
      }
      const file = `./data/${selected}.json`;
      const res = await fetch(`${file}?t=${Date.now()}`, { cache: "no-store" });
      if (!res.ok) {
        showOtherReportMessage(
          `아직 이 경기 CHALK BOARD 데이터가 없습니다.\n` +
      ...
      if (!data?.events?.length) {
        showOtherReportMessage("경기 파일은 있지만 이벤트가 비어 있습니다.", true);
        return;
      }
      state.otherData = data;
      renderOtherReport(data);
      setStatus("");
    } catch (err) {
      console.error(err);
      showOtherReportMessage(String(err.message || err), true);
    } finally {
      if (btn) {
        btn.disabled = false;
  ...
  async function boot() {
    ...
    let listError = null;
    try {
      await Promise.all([loadIndex(false), loadSchedule(false)]);
      fillOtherYearRound();
    } catch (err) {
      console.error(err);
      listError = err;
