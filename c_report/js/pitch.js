...
const Pitch = (() => {
  ...
  function render(canvas, opts) {
    ...
    if (opts.mode === "shape" && opts.nodes?.length) {
      ...
      /* Attack-direction marker so the orientation is never ambiguous. */
      ctx.fillStyle = "rgba(255,255,255,0.65)";
      ctx.font = `600 ${Math.max(9, cssW * 0.015)}px IBM Plex Sans KR, sans-serif`;
      ctx.textAlign = "right";
      ctx.textBaseline = "bottom";
      ctx.fillText("공격 방향 →", cssW - geom.pad - 4, cssH - geom.pad - 6);
      if (opts.hover) bindHover(canvas, hits);
    }
