...
const Tactics = (() => {
  const LANES = [
    ...
    { key: "lh", label: "왼쪽 안쪽", short: "좌안쪽", from: 20, to: 37 },
    { key: "c", label: "중앙", short: "중앙", from: 37, to: 63 },
    { key: "rh", label: "오른쪽 안쪽", short: "우안쪽", from: 63, to: 80 },
  ...
  function roleLabel(node, stat, frame) {
    ...
    if (position === "DF") {
      if (wide) return x >= backLine + 6 ? `${sideWord} 공격형 풀백` : `${sideWord} 풀백`;
      return x >= backLine + 5 ? "전진형 센터백" : "센터백";
    }
    if (position === "FW") {
    ...
    if (rel <= -6) return stat.progressive >= 8 ? "뒤에서 키패스하는 미드" : "수비형 미드필더";
    if (rel < 4) {
      if (wide) return `${sideWord} 미드필더`;
      return stat.defActions >= 12 ? "수비부터 공격까지 뛰는 미드" : "중앙 미드필더";
    }
    if (wide) return stat.boxTouches >= 6 ? `${sideWord} 안으로 접는 윙어` : `${sideWord} 윙어`;
    if (halfSpace) return `${sideWord} 안쪽 공격형 미드`;
    return "공격형 미드필더";
  }
