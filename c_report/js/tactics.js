...
const Tactics = (() => {
  const LANES = [
    ...
    { key: "lh", label: "왼쪽 하프스페이스", short: "좌하프", from: 20, to: 37 },
    { key: "c", label: "중앙", short: "중앙", from: 37, to: 63 },
    { key: "rh", label: "오른쪽 하프스페이스", short: "우하프", from: 63, to: 80 },
  ...
  function roleLabel(node, stat, frame) {
    ...
    if (rel <= -6) return stat.progressive >= 8 ? "딥라잉 플레이메이커" : "홀딩 미드필더";
    if (rel < 4) {
      if (wide) return `${sideWord} 미드필더`;
      return stat.defActions >= 12 ? "박스 투 박스" : "중앙 미드필더";
    }
    if (wide) return stat.boxTouches >= 6 ? `${sideWord} 인사이드 윙어` : `${sideWord} 윙어`;
    if (halfSpace) return `${sideWord} 하프스페이스 공격형 MF`;
    return "공격형 미드필더";
  }
