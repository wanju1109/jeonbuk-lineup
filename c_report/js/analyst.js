...
const Analyst = (() => {
  ...
  function possessionModel(team) {
    ...
    if (seq.avgPasses >= 3.2 && prog.avgPassLength <= 19) return "짧은 패스로 상대를 끌어내며 공을 지키는 쪽";
    if (seq.avgPasses <= 1.6 && prog.avgPassLength >= 20) return "한두 번 터치하고 바로 앞으로 넘기는 쪽";
    if (seq.avgPasses <= 2.2) return "공을 오래 쥐기보다 빠르게 앞으로 보내는 쪽";
    return "공을 지키는 것과 앞으로 넘기는 것을 섞는 쪽";
  }

  function pressModel(team) {
    const p = team.pressing;
    if (p.ppda !== null && p.ppda <= 9 && p.defLineHeight >= 44) return "상대 진영에서부터 달라붙는 강한 압박";
    if (p.ppda !== null && p.ppda <= 13) return "하프라인 근처에서 조이는 압박";
    if (p.defLineHeight <= 33) return "자기 골문 앞에 내려앉아 지키는 수비";
    return "중원에서 선을 긋고 지키는 수비";
  }
  ...
  function chapterModels(ctx) {
    ...
    paragraphs.push(
      ...
      possessionModel(home).includes("지키는")
        ? `쉽게 말하면 ${homeName}은 공을 오래 돌리며 상대가 지치길 기다리는 쪽에 가깝습니다. 상대가 쫓아오다 빈 공간이 생기면 그때 찌르는 그림이죠.`
        : possessionModel(home).includes("넘기는")
          ? `쉽게 말하면 ${homeName}은 공을 오래 쥐기보다, 한두 번에 앞으로 넘기며 상대가 자리를 잡기 전에 승부를 보려 했습니다.`
          : `${homeName}은 상황에 따라 공을 지키기도, 빠르게 넘기기도 했습니다. 상대가 어떻게 나오느냐에 맞춰 템포를 바꾼 날입니다.`,
      `${topic(awayName)} ${possessionModel(away)}이었습니다. 수비할 때는 ${pressModel(away)}입니다.`,
      `두 팀의 의도가 부딪힌 지점은 이겁니다. ${homeName}이 공을 가지고 있을 때, ${awayName}가 어느 높이에서 끊으려 했느냐. 앞에서 끊으면 바로 역습이 되고, 내려앉아 끊으면 시간이 걸립니다.`
    );

    paragraphs.push(
      `상대 골문 근처에서 공을 만진 비율은 ${homeName} ${pctText(tilt.home)}, ${awayName} ${pctText(tilt.away)}입니다.`,
    ...
    return {
      ...
      paragraphs,
      metrics: [
        chip("골문 근처 점유", pctText(tilt.home), pctText(tilt.away), "상대 골문 앞에서 공을 만진 비율"),
  ...
  function chapterShape(ctx) {
    ...
    paragraphs.push(
      `선발 11명이 90분 동안 서 있던 평균 자리를 세로로 묶으면, ${homeName}은 ${hf.label ? `${hf.label} 형태` : "뚜렷이 나뉘지 않는 형태"}로 보입니다.`,
      `${awayName}는 ${af.label ? `${af.label} 형태` : "뚜렷이 나뉘지 않는 형태"}입니다.`,
  ...
  function chapterBuildUp(ctx) {
    ...
    return {
      ...
      paragraphs,
      metrics: [
        chip("앞으로 간 패스", `${hp.progressive} (${pctText(hp.progressiveRate)})`, `${ap.progressive} (${pctText(ap.progressiveRate)})`, "12m 이상 전방으로 이동"),
  ...
  function chapterFinalThird(ctx) {
    ...
    paragraphs.push(
      `페널티박스 정면(골문과 가장 가까운 가운데 공간)으로 들어간 패스는 ${homeName} ${hp.zone14}회, ${awayName} ${ap.zone14}회입니다.`,
      `슈팅으로 이어진 키패스는 ${hp.keyPasses}회 대 ${ap.keyPasses}회입니다.`,
    ...
    if (crossHeavy && cheapShots) {
      paragraphs.push(
        ...
        `박스 정면으로 들어간 패스는 ${hp.zone14}회에 그쳤습니다. 골문과 가장 가까운 가운데를 잘 못 썼다는 뜻입니다.`,
      ...
      paragraphs.push(
        ...
        `같은 자리에서 100번 때리면 약 6골 (xG 0.06)보다 어려운 슈팅이 ${hs.poor}개(${pctText(hs.poorRate)})입니다. 슈팅 숫자는 착시일 수 있습니다.`
      );
    } else {
      paragraphs.push(
        `${homeName}의 슈팅 하나당 평균은 ${xgTalk(hs.avgXg)}, ${awayName}는 ${xgTalk(as.avgXg)}입니다.`,
    ...
    paragraphs.push(
      ...
      `같은 자리에서 100번 때리면 약 25골 (xG 0.25) 이상의 결정적 기회는 ${hs.big}개 대 ${as.big}개였습니다.`
    );
    ...
    return {
      ...
      metrics: [
        ...
        chip("박스 정면 진입", `${hp.zone14}회`, `${ap.zone14}회`, "골문 앞 가운데 공간"),
        chip("슈팅당 xG", fmt(hs.avgXg, 2), fmt(as.avgXg, 2), "같은 자리에서 100번 때리면 약 몇 골"),
      ],
    };
  }
  ...
  function chapterPressing(ctx) {
    ...
    paragraphs.push(
      `상대가 패스를 몇 번 하는 동안 우리가 한 번 달려들었는지(PPDA)는 ${homeName} ${ppdaText(hp)}, ${awayName} ${ppdaText(ap)}입니다.`,
    ...
    paragraphs.push(
      `수비가 가장 자주 개입한 길은 ${homeName}이 ${hTop.lane.label}(${pctText(hTop.share)}), ${awayName}가 ${aTop.lane.label}(${pctText(aTop.share)})입니다.`,
    ...
    return {
      ...
      paragraphs,
      metrics: [
        chip("PPDA", ppdaText(hp), ppdaText(ap), "낮을수록 강하게 눌렀음"),
  ...
  function chapterTransition(ctx) {
    ...
    paragraphs.push(
      ...
      `역습은 상대가 자리를 잡기 전이라, 같은 슈팅이라도 골이 될 확률이 높게 나오는 구간입니다. 상대가 “아직 안 모였어!” 하는 그 3초를 훔치는 싸움입니다.`
    );
  ...
  function chapterMatchups(ctx) {
    ...
    if (main && main.lane) {
      ...
      paragraphs.push(
        `${homeName}이 상대 골문 앞 30m에 들어간 길을 다섯으로 나누면, 가장 많이 쓴 길은 ${main.lane.label}입니다.`,
    ...
    if (awayMain && awayMain.awayLane && awayMain.awayAttack > 0) {
      paragraphs.push(
        `${awayName}가 상대 골문 앞에 들어간 길은, 자기 기준으로 ${awayMain.awayLane.label}이 ${awayMain.awayAttack}회(${pctText(awayMain.awayAttackShare)})로 가장 많았습니다.`,
  ...
  function chapterPhases(ctx) {
    ...
    paragraphs.push(
      `15분 단위로 골이 될 확률 합을 끊어 보면, ${homeName}의 최고 구간은 ${bestHome.from}~${bestHome.to}분입니다. 그때 확률 합은 ${fmt(bestHome.home, 2)} 대 ${fmt(bestHome.away, 2)}입니다.`,
    ...
    if (momentum.goals.length) {
      ...
      paragraphs.push(
        `득점 시점은 ${g}입니다.`,
        `누적 골 확률 합은 ${homeName} ${fmt(momentum.homeTotal, 2)}, ${awayName} ${fmt(momentum.awayTotal, 2)}로 끝났습니다.`,
  ...
  function chapterSetPieces(ctx) {
    ...
    paragraphs.push(
      ...
      `세트피스에서 나온 슈팅은 ${hs.setPieceShots}개(골 확률 합 ${fmt(hs.setPieceXg, 2)}) 대 ${as.setPieceShots}개(${fmt(as.setPieceXg, 2)})입니다.`
    );

    if (hs.corners >= 6 && hs.setPieceXg < 0.3) {
      paragraphs.push(
        `${topic(homeName)} 코너킥을 ${hs.corners}개나 얻고도 세트피스 골 확률 합이 ${fmt(hs.setPieceXg, 2)}에 그쳤습니다.`,
        `흐름에서 막힌 팀에게 세트피스는 가장 값싼 득점 경로입니다. 여기서 확률이 쌓이지 않는다면 키커와 움직임 루틴을 다시 설계해야 합니다.`
      );
    } else if (as.penalties) {
      paragraphs.push(
        `${topic(awayName)} 페널티킥을 얻었습니다. PK 한 번의 골 확률은 약 0.78로, 흐름에서 만드는 열 번의 슈팅과 맞먹습니다.`,
    ...
    return {
      ...
      metrics: [
        ...
        chip("세트피스 골 확률", fmt(hs.setPieceXg, 2), fmt(as.setPieceXg, 2), "정지 상황 기대득점"),
  ...
  function chapterIndividuals(ctx) {
    ...
    if (engines.length) {
      ...
      if (named.has(first.playerId)) {
        ...
        paragraphs.push(
          `앞으로 보내는 패스도 같은 선수가 최다입니다(${first.name} ${first.stat.progressive}회).`,
        ...
        paragraphs.push(
          `앞으로 보내는 패스를 실제로 담당한 선수는 ${tag(first)}입니다. 전진 패스 ${first.stat.progressive}회입니다.`,
    ...
    if (danger) {
      paragraphs.push(
        ...
        `슈팅 ${danger.stat.shots}회, 골 확률 합 ${fmt(danger.stat.xg, 2)}, 득점 ${danger.stat.goals}골입니다.`,
        danger.stat.goals >= 2
  ...
  function chapterPrescription(ctx) {
    ...
    if (hp.crossShare >= 55 && hp.intoBox >= 15) {
      items.push(
        `크로스 의존을 낮춰야 합니다. 박스 침투 ${hp.intoBox}회 중 ${hp.crossIntoBox}회가 크로스였고, 박스 정면 진입은 ${hp.zone14}회뿐입니다. 측면에서 올리기 전에 안쪽으로 한 번 접어 들어가는 3인 조합(윙어–중앙 미드–풀백 오버랩)을 훈련 우선순위로 잡아야 합니다.`
      );
    }
    ...
    if (home.setPieces.corners >= 6 && home.setPieces.setPieceXg < 0.3) {
      items.push(
        `코너킥 ${home.setPieces.corners}개에서 세트피스 골 확률 합이 ${fmt(home.setPieces.setPieceXg, 2)}입니다. 가까운 포스트로 꺾어 주는 움직임과, 떨어진 공 자리를 다시 짤 여지가 큽니다.`
      );
    }
    if (as.avgXg > hs.avgXg + 0.05) {
      items.push(
        `${topic(awayName)} 더 적은 슈팅으로 더 좋은 자리를 잡았습니다. ${awayName}은 ${xgTalk(as.avgXg)}, ${homeName}은 ${xgTalk(hs.avgXg)}. 상대가 어떤 길로 박스 중앙에 도달했는지 영상으로 되짚고, 그 길의 첫 단추를 차단하는 것이 수비 과제입니다.`
      );
    }
    if (!items.length) {
      items.push(
