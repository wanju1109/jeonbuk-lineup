  ...
  function chapterStaff(ctx) {
    ...
    if (hx && (hx.occupancy.wingRate >= 48 || hp.crossShare >= 50) && hp.zone14 <= 8) {
      constraints.push({
        title: "내부 진입",
        body:
          `하프스페이스 점유 ${pctText(hx.occupancy.halfSpaceRate)}, 존 14 ${hp.zone14}회, 컷백 ${hx.cutbacks}회. ` +
          `15분 게임: 박스 진입은 존 14 또는 컷백만 인정. 하프스페이스 3인(풀백–8–윙) 트라이앵글을 오른쪽에 고정한다.`,
      });
    }
    if (hs.outsideRate >= 28 && hs.shots >= 10) {
      constraints.push({
        title: "박스 밖 슈팅 금지",
        body:
          `박스 밖 비율 ${pctText(hs.outsideRate)}. 그 자리에서 슈팅 시 상대 볼. 한 터치 더 가지고 컷백 각을 찾게 한다.`,
      });
    }
    if (hx && hx.betweenLine <= 6 && home.sequences.avgPasses >= 2.4) {
      constraints.push({
        title: "수신 높이",
        body:
          `라인 사이 ${hx.betweenLine}회. 10번·8번의 첫 터치가 상대 6번 앞에서 일어나면 리셋. ` +
          `백라인과 미드 라인 사이에서만 받아도 된다고 규칙을 건다.`,
      });
    }
    if (hx && hx.restBreachRate >= 22 && hx.restLoss >= 4) {
      constraints.push({
        title: "레스트 디펜스 3인",
        body:
          `붕괴 ${hx.restBreach}/${hx.restLoss}. 볼이 공격 지역에 있는 동안 CB–CB–6 삼각형이 해체되면 즉시 휘슬. ` +
          `풀백 오버랩은 6번이 볼과 골문 사이에 있을 때만 허용.`,
      });
    }
    if (hx?.hub?.share >= 22) {
      constraints.push({
        title: "세컨드 허브",
        body:
          `${tag(hx.hub.player)} 집중도 ${pctText(hx.hub.share)}. ` +
          `이 선수를 지나지 않는 전진 시퀀스만 득점으로 인정하는 빌드업 게임을 한 세트 넣는다.`,
      });
    }
    if (home.pressing.defLineHeight >= 44 && away.sequences.fastShots >= 4) {
      constraints.push({
        title: "프레스 깨진 뒤의 각",
        body:
          `라인 높이 ${fmt(home.pressing.defLineHeight)}, 상대 속공 슈팅 ${away.sequences.fastShots}회. ` +
          `첫 압박이 실패한 순간 커버 미들의 위치를 콘으로 고정하고, 그 콘을 지나 역습이 나오면 실점으로 친다.`,
      });
    }
    if (hx && hx.front.yRate < 18 && hx.front.t >= 80) {
      constraints.push({
        title: "수직",
        body:
          `전방패스 Y ${pctText(hx.front.yRate)}, 횡(T) ${hx.front.t}회. 완성 패스 중 T는 점유, Y만 진행이다. ` +
          `한 시퀀스에 Y가 없으면 리셋하는 빌드업 게임을 넣는다.`,
      });
    }
    if (hx && hx.aerial.total >= 10 && hx.aerial.winRate <= 40 && home.progression.crossShare >= 45) {
      constraints.push({
        title: "약한 공중을 그만 보낸다",
        body:
          `공중 승률 ${pctText(hx.aerial.winRate)}인데 크로스 의존 ${pctText(home.progression.crossShare)}. ` +
          `크로스 대신 컷백만 득점인 게임을 15분.`,
      });
    }
    if (hx && hx.shots.blockRate >= 22 && hx.shots.block >= 3) {
      constraints.push({
        title: "블락된 슈팅",
        body: `${pctText(hx.shots.blockRate)}가 첫 수비수에 맞았다. 컷백 수신 첫 터치 슈팅만 인정.`,
      });
    }
    if (hx && hx.setpiece.corners >= 5 && hx.setpiece.cornerRate < 20) {
      extras.push({
        ...
        title: "루틴",
        body:
          `코너 ${hx.setpiece.corners}개 → 슈팅 ${pctText(hx.setpiece.cornerRate)}. 니어 스크린과 세컨드볼 존을 다시 짠다.`,
      });
    }
    if (away.shooting.avgXg > hs.avgXg + 0.04) {
      extras.push({
        ...
        title: "과제",
        body:
          `${topic(awayName)} 슈팅당 xG ${fmt(away.shooting.avgXg, 3)} vs ${homeName} ${fmt(hs.avgXg, 3)}. ` +
          `상대가 존 14·컷백 중 어느 각으로 그 질을 만들었는지 첫 패스부터 차단한다.`,
      });
    }
    if (!constraints.length && !extras.length) {
      extras.push({
        tag: "노트",
        title: "",
        body:
    ...
    const numbered = constraints.map((c, i) => ({
      tag: `제약${i + 1}`,
      title: c.title,
      body: c.body,
    }));
    const closing = {
      ...
      body:
        meta?.score && meta.score.home !== undefined
          ? `${meta.score.home}-${meta.score.away}. 스코어는 한 번이다. 위의 경로는 다음 주에도 상대가 준비해 온다.`
    ...
    return {
      ...
      paragraphs: numbered.concat(extras).concat([closing]),
    };
  }
      ...
      try {
        ...
        if (ch && ch.paragraphs && ch.paragraphs.length) {
          ch.paragraphs = ch.paragraphs.filter((p) => {
            if (p && typeof p === "object") return Boolean(String(p.body || "").trim() || p.tag);
            return Boolean(String(p || "").trim());
          });
          chapters.push(ch);
        }
      } catch (err) {
...
const ExpertView = (() => {
  ...
  function paragraphHtml(p) {
    if (p && typeof p === "object" && p.tag) {
      const title = p.title
        ? `<strong class="ex-staff-title">${escapeHtml(p.title)}</strong>. `
        : "";
      return (
        `<p class="ex-staff-item" data-kind="${escapeHtml(p.tag)}">` +
        `<span class="ex-staff-tag">[${escapeHtml(p.tag)}]</span>` +
        `<span class="ex-staff-copy">` +
        title +
        escapeHtml(p.body || "") +
        `</span>` +
        `</p>`
  ...
  function renderChapters(ctx, chapters) {
    ...
    box.innerHTML = chapters
      .map((ch) => {
        const paras = ch.paragraphs.map((p) => paragraphHtml(p)).join("");
        return (
