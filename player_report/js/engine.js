...
const PlayerEngine = (() => {
  ...
  const CURATED_YEARS = {
    "20180025": {
      ...
      "2022": "35경기 연속 주전. 커리어 중반의 안정 구간이다. 위치 선정과 커맨드가 반사신경보다 먼저 나오는 타입으로 자리 잡았다. 시즌이 끝난 뒤 FA로 J리그 쇼난 벨마레에 갔다. K리그 공식 표가 2023–24에 비는 이유다.",
      "2025": "쇼난 벨마레 2년 뒤 전북 복귀 첫 풀시즌. 군이 아니라 J리그였다. 38경기·클린시트 15. 수비 블록 호흡을 되찾는 초반을 지나 다시 전북의 마지막 줄이 됐다.",
  ...
  function seasonNote(p, row, prev) {
    ...
    if (gk) {
      ...
      if (prevT && prevT.apps) {
        ...
        else if (!inProgress && dApps <= -6) bits.push("전년보다 출장이 줄었다. 단축 시즌·이적·로테이션·경쟁 구도 중 하나다.");
