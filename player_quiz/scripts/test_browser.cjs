...
(async()=>{const browser=await chromium.launch({headless:true,channel:'msedge'});try{
 ...
 await page.getByRole('heading',{name:'시간 초과! 아쉽지만, 정답은…'}).waitFor({timeout:11500});
 ...
 await page.click('#copy-result');const result=await page.evaluate(()=>navigator.clipboard.readText());assert(result.includes('3/5')&&result.includes(url)&&result.includes('10초'));
